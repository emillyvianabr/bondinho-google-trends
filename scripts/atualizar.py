...
def client() -> TrendReq:
    # O retry interno do pytrends 4.9.2 usa o argumento removido
    # `method_whitelist` do urllib3. As tentativas são controladas por
    # request_with_retry(), portanto deixamos o retry interno desativado.
...
def existing_series():
    if not DATA_XLSX.exists():
        return pd.DataFrame()
    old = pd.read_excel(DATA_XLSX, sheet_name="serie_mensal")
    old["data"] = pd.to_datetime(old["data"])
    return old
...
def collect_series(py):
    ...
    for code, (market, geo) in MARKETS.items():
        try:
            ...
            fallback = old[old["mercado_codigo"] == code]
            if fallback.empty:
                raise RuntimeError(f"Sem coleta nem histórico para {market}: {exc}") from exc
        ...
        try:
            ...
            for dt, values in both.iterrows():
                comparison.append({"date": dt.strftime("%Y-%m-%d"), "year": dt.year,
                                   ...
                                   "cristo": int(values[COMPARE[0]]), "bondinho": int(values[TOPIC_ID])})
        except Exception as exc:
...
def existing_geo():
    if not DATA_XLSX.exists():
        return pd.DataFrame()
    book = pd.ExcelFile(DATA_XLSX)
    sheets = [name for name in book.sheet_names if name.startswith("geo_") and name != "geo_historico"]
    if not sheets:
        return pd.DataFrame()
...
def write_outputs(series, geo, comparison):
    ...
    compared_markets = {row["marketCode"] for row in comparison}
    if set(MARKETS).issubset(compared_markets):
        ...
        print("AVISO: comparação incompleta; comparison-data.js anterior preservado.")
...
def main():
    ...
    try:
        ...
        if geo.empty:
            raise
        print(f"AVISO: GeoMap sem coleta nova; snapshot anterior preservado. Motivo: {exc}")
    if len(series) < 100 or geo.empty:
